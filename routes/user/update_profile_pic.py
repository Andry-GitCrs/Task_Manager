...
def update_profile_pic(app, database):
    db = database["db"]
    ...
    def _update_profile_pic():
        ...
        if user is None:
            abort(401)
        user.profile_pic = unique_name      # store only the file name
        db.session.commit()

        # --------- respond with public URL ----------
        public_url = url_for("static", filename=f"uploads/profile/{unique_name}")
        return jsonify(message="Profile picture updated", profile_pic=public_url), 200
