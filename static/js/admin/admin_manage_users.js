...
function user_card(user, activityColor, activityText) {
    ...
    col.innerHTML = user_row(user, activityColor, activityText);
    const total = user.tasks_count + user.user_subtasks_count;
...
async function deleteUser(userId) {
    if (await showFlexibleModal('Are you sure you want to delete this user')) {
        document.getElementById("loading").style.display = 'inline';
        try{
...
adduser_form.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (response.ok) {  //Response with status code 200
            ...
            const userCardsWrapper = document.getElementById('userCardsWrapper')
            const card = user_card(user, 'danger', '0%')
            ...
            userCardsWrapper.appendChild(card)
